...
| `self-test` | 회사 코드 없이 도구 자체 정상동작 검증 (115개 체크) | X | X |
...
이 파일은 요약, 단계별 범위, 상위 Focus Queue, 런타임 수동 검증 시나리오, selector/page 매칭, AJAX/서버 매핑, 다음 액션을 `KEY|field=value` 형식으로 압축합니다.
