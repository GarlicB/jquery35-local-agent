...
- **로컬 웹 콘솔(ui)** — PowerShell 명령을 매번 치지 않고 브라우저에서 source만 지정하면 TO-BE/report/profile/server-source/verify-source를 자동 세팅하고, One Button Pipeline, Local Lab 시작/중지, 기존 HTML 보고서 열기를 처리
...
- source를 지정하면 TO-BE/report/profile/server-source/verify-source가 자동 산정됩니다. `Auto setup`은 같은 자동 산정을 다시 강제 적용하고, `Run Pipeline`은 분석 → TO-BE 자동수정 → jQuery 교체 → Hermes 검수팩 → PR/CI 보고서 → 폐쇄망 증빙 → Local Lab 시작 순서로 실행합니다.
...
| `self-test` | 회사 코드 없이 도구 자체 정상동작 검증 (103개 체크) | X | X |
