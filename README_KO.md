...
- **기본 jQuery 런타임 번들 포함** — `patch-jquery`가 `assets/jquery/jquery-3.5.1.min.js`와 `jquery-migrate-3.6.0.min.js`를 TO-BE에 자동 배치하므로, 기본 조합은 별도 파일 준비 없이 교체 가능
...
| `self-test` | 회사 코드 없이 도구 자체 정상동작 검증 (126개 체크) | X | X |
...
7.  실행3_jquery교체시도.bat  (mode patch-jquery, 커밋 그룹: JQUERY_CORE)
    - 기본값은 도구에 포함된 `assets/jquery/jquery-3.5.1.min.js` / `jquery-migrate-3.6.0.min.js`를 TO-BE `WebContent\js`에 자동 배치합니다.
    - 별도 승인된 사내 배포 파일을 써야 하면 `project-profile.json`의 `jquery.coreFile`, `jquery.migrateFile`, `jquery.newJquerySrc`, `jquery.newMigrateSrc`를 맞추고 해당 파일을 target에 직접 배치하세요.
...
기본 `jquery-3.5.1.min.js` / `jquery-migrate-3.6.0.min.js`는 도구가 TO-BE에 자동 배치합니다. 그래도 SKIP이면 profile에서 커스텀 파일명/경로를 지정했거나 CDN/외부 URL이라서 자동 배치 대상이 아닌 경우입니다. CDN 태그는 자동 교체하지 않고 MANUAL로 남깁니다.
