...
- **Hermes 로컬 검수팩(hermes-pack)** — 외부 AI에게 묻기 전/후에 로컬에서 확인할 테스트 설계, 검수 기준, project-profile 반영 템플릿을 생성하는 폐쇄망용 부품팩
...
| `self-test` | 회사 코드 없이 도구 자체 정상동작 검증 (78개 체크) | X | X |
...
3. 동시에 `hermes_test_plan.md`, `hermes_review_matrix.csv`, `hermes_profile_patch.sample.json`이 생성됩니다. 이 파일들은 폐쇄망 내부에서 각 CASE의 정적 확인 포인트, Probe/Lab 테스트, 통과/실패 기준을 검수하는 용도입니다.
