...
- **로컬 웹 콘솔(ui)** — PowerShell 명령을 매번 치지 않고 브라우저에서 경로 저장, 모드 실행, Local Lab 시작/중지, 기존 HTML 보고서 열기를 처리
...
- `source`, `target`, `report`, `profile`, `rulepack`, `server-source`를 저장한 뒤 버튼으로 `plan`, `autofix`, `patch-jquery`, `probe`, `review-pack`, `hermes-pack`, `verify-clean`, `pr-report`, `airgap-manifest`, `release-zip`을 실행합니다.
