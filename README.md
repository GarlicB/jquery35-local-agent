...
- **기본 jQuery 런타임 번들** — `patch-jquery`가 `jquery-3.5.1.min.js`와 `jquery-migrate-3.6.0.min.js`를 TO-BE 트리에 자동 배치하므로 기본 조합은 별도 파일 준비 없이 실행 가능합니다.
...
- **자체 검증 내장** — `--mode self-test`가 임시 샘플 프로젝트를 만들어 전체 사이클을 검증. v5.6.6은 source-only 자동 세팅, S1~S10 웹 콘솔 단계명, 경로/유형 집계, AS-IS/TO-BE 줄바꿈 보존 자동수정, 번들 jQuery 자동 배치, UI 요소 기반 런타임 시나리오, Runtime Parity 분석, Voyager 복붙 패킷 생성을 포함합니다.
