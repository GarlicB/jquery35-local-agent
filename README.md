...
- **자체 검증 내장** — `--mode self-test`가 임시 샘플 프로젝트를 만들어 전체 사이클(102개 체크)을 검증. v5.6.0은 경로/유형 집계, 웹 콘솔 파이프라인, AS-IS/TO-BE 줄바꿈 보존 자동수정을 포함합니다.
...
브라우저 콘솔은 `http://127.0.0.1:18088/` 에서 열립니다. 여기서 경로를 Browse로 지정하면 TO-BE/report 기본 경로가 자동 산정되고, 버튼 또는 One Button Pipeline으로 실행하면 됩니다.
