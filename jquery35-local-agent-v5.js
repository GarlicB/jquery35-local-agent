...
function kpiCard(label, value, color) {
  return '<div class="card" style="--accent:' + color + '"><div class="v">' + htmlEsc(value) + '</div><div class="l">' + htmlEsc(label) + "</div></div>";
}
...
function dashboardUiHtml() {
  ...
  return "<!DOCTYPE html><html lang=\"ko\"><head><meta charset=\"utf-8\"><title>JQ35 Local Console</title><style>" +
    ...
    "function busy(p){uiBusy++;return p.then(function(d){uiBusy--;return d},function(e){uiBusy--;throw e})}" +
    "function saveConfig(auto){var o=collect();if(auto)o.auto=true;return busy(api('/api/config',{method:'POST',body:JSON.stringify(o)}).then(function(d){state=d.state;fill();return d}))}" +
    "function autoSetupFromSource(force){var src=byId('cfg_source').value;if(!src)return Promise.resolve();return busy(api('/api/default-paths',{method:'POST',body:JSON.stringify({source:src,createProfile:true})}).then(function(d){['source','target','report','profile','rulepack','serverSource','verifySource'].forEach(function(k){if(d[k]&&(force||k==='source'||!byId('cfg_'+k).value))byId('cfg_'+k).value=d[k]});return saveConfig(true)}))}function derivePaths(force){return autoSetupFromSource(force)}" +
    ...
    "function refresh(){if(uiBusy>0)return;api('/api/state').then(function(d){state=d.state;job=d.job;lab=d.lab;fill();renderLinks(d.files);renderStatus()})}setInterval(refresh,2000);refresh();" +
    "</script></body></html>";
}
